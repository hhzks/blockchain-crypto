...
class Blockchain {
private:
    std::vector<std::shared_ptr<Block>> chain;              // The blockchain
    std::vector<std::shared_ptr<Transaction>> pendingTransactions; // Pending transactions
    int difficulty;                                         // Mining difficulty
    double miningReward;                                    // Reward for mining a block
    std::unordered_map<std::string, double> balances;      // Account balances
