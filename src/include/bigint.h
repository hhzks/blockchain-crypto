...
inline BigInt::BigInt(const std::string& num, int base) {
    if (base < 2 || base > 36) {
...
inline std::string BigInt::to_binary_string() const{
...
inline BigInt BigInt::operator/(const BigInt& num) const {
    ...
    if ((this->sign == '-'  && num.sign == '+') || (this->sign == '+'  && num.sign == '-')){
        abs_dividend = abs(*this) - 1;
        abs_divisor = abs(num);
        is_negative = true;
    }
    else {
        abs_dividend = abs(*this);
        abs_divisor = abs(num);
        is_negative = false;
    }
